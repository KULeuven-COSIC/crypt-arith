...
module Bmult14x14_bitheap_cmp_tb ();
    // ============================== parameters ==============================
    `define CLK_P         10
    `define CLK_HP        5
    `define TS_SIZE       1000
    ...
    logic [7  : 0] col14_ts [`TS_SIZE-1 : 0];
    logic [6  : 0] col15_ts [`TS_SIZE-1 : 0];
    logic [5  : 0] col16_ts [`TS_SIZE-1 : 0];
    logic [5  : 0] col17_ts [`TS_SIZE-1 : 0];
    logic [4  : 0] col18_ts [`TS_SIZE-1 : 0];
    logic [4  : 0] col19_ts [`TS_SIZE-1 : 0];
    logic [3  : 0] col20_ts [`TS_SIZE-1 : 0];
    logic [3  : 0] col21_ts [`TS_SIZE-1 : 0];
    logic [2  : 0] col22_ts [`TS_SIZE-1 : 0];
    logic [2  : 0] col23_ts [`TS_SIZE-1 : 0];
    logic [1  : 0] col24_ts [`TS_SIZE-1 : 0];
    logic [1  : 0] col25_ts [`TS_SIZE-1 : 0];
    logic          col26_ts [`TS_SIZE-1 : 0];
    logic          col27_ts [`TS_SIZE-1 : 0];
    logic [28 : 0] comp_out_ts [`TS_SIZE-1 : 0];
    ...
    initial begin
        ...
        for (j = 0; j < `TS_SIZE; j = j + 1) begin
            if (comp_out[27:0] == comp_out_ts[j][27:0]) begin
                $display("Testvector-%d CORRECT!", j);
                correct_cnt = correct_cnt + 1;
            end else begin
                $display("=================================================================================");
                $display("Testvector-%d WRONG", j);
                $display("module    output: %b", comp_out[27:0]);
                $display("reference output: %b", comp_out_ts[j][27:0]);
                $display("difference:       %b", comp_out_ts[j][27:0] - comp_out[27:0]);
                $display("=================================================================================");
            end
            #`CLK_P;
        end
